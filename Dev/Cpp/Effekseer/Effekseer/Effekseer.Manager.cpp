...
void ManagerImplemented::Update(float deltaFrame)
{
	...
	{
#if 1
		const uint32_t chunkStep = m_WorkerThreads.size() + 1;

		for (uint32_t workerID = 0; workerID < (uint32_t)m_WorkerThreads.size(); workerID++)
		{
			const uint32_t chunkOffset = 1 + workerID;
		...
		for (size_t workerID = 0; workerID < m_WorkerThreads.size(); workerID++)
		{
...
ManagerImplemented::WorkerThread::WorkerThread()
{
...
void ManagerImplemented::WorkerThread::RunAsync(std::function<void()> task)
{
	std::unique_lock<std::mutex> lock(m_Mutex);
	m_Task = task;
...
void ManagerImplemented::WorkerThread::WaitForComplete()
{
	std::unique_lock<std::mutex> lock(m_Mutex);
